/**
 * Browser entry point of RACS (Remote Agent Context Store).
 *
 * Re-exports the full public surface EXCEPT the Node-only file state backend, so browser
 * bundles never advertise `fileState`. Persist through `memoryState`, or `kvState` over any
 * key-value store the host already reaches.
 *
